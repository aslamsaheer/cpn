...
function renderSaved(){let list=offers.filter(x=>saved.includes(x.name));$("savedList").innerHTML=list.length?list.map(x=>`<div class="saved-card" onclick="openOffer(${offers.indexOf(x)})"><img src="${imgUrl(x)}" onerror="this.src='assets/'+(offers.find(o=>o.name===this.alt)?.img||'restaurant.svg')" alt="${x.name}"><div><b>${x.name}</b><small>★ ${x.rating} · ${x.reviews} reviews</small><small>${x.tag}</small></div><div class="offer">${x.offer}</div></div>`).join(""):`<div class="empty">No saved offers yet.<br>Tap ♡ Save on an offer.</div>`}
function renderClaims(){ $("claimedList").innerHTML=claimed.length?claimed.map(c=>`<div class="saved-card"><img src="${imgUrl(offers.find(x=>x.name===c.name)||{img:'restaurant.svg'})}" alt="${c.name}"><div><b>${c.name}</b><small>${c.offer} · ${c.time}</small><small>Redemption ID: ${c.code}</small></div></div>`).join(""):`<div class="empty">You haven't claimed an offer yet.</div>`}
function setup(){
